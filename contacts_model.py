...
class Contact:
    ...
    def search(cls, text):
        ...
        for c in cls.db.values():
            # Convert each field to lowercase if it's not None
            match_first = c.first is not None and text in c.first.lower()
            match_last = c.last is not None and text in c.last.lower()
            match_email = c.email is not None and text in c.email.lower()
